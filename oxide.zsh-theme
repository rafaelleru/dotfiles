...
env_info() {
	if [ ! -z "$IDTHEFT_TESTS_TARGET" ]; then
		env_target="%F{white}% using %{$oxide_orange%}% $IDTHEFT_TESTS_TARGET"
	fi
}

add-zsh-hook precmd env_info

# Oxide prompt style.
PROMPT=$'\n%{$oxide_limegreen%}%~%{$oxide_reset_color%} ${vcs_info_msg_0_} %{$env_target%}% \n%(?.%{%F{white}%}.%{$oxide_red%})%(!.#.❯)%{$oxide_reset_color%} '
